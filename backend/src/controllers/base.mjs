...
import QueryGenerator from '../utils/QueryGenerator.mjs'
...
const getAll =
  ...
  async (req, res, next) => {
    try {
      const { mongoQuery, query } = new QueryGenerator(Model, req.query)
        .find()
        .sort()
        .paginate()
        .limitFields()
...
const getById =
  ...
  async (req, res, next) => {
    try {
      const { mongoQuery, query } = new QueryGenerator(
      ...
      if (!doc) {
        const err = new HttpError(404, 'Không tìm thấy bản ghi với id đã cho')
